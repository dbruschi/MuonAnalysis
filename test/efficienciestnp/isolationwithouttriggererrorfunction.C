...
void isolationtnp(const char* name, const char* name2) {
	gStyle->SetOptStat(0);
...
//	d1=d1.Define("goodidippt",goodtrackgen,{"triggerdenominatortag","triggerdenominatorprobe","Muon_genPartIdx","goodgenidx","goodgenpt"}).Define("goodidipeta",goodtrackgen,{"triggerdenominatortag","triggerdenominatorprobe","Muon_genPartIdx","goodgenidx","goodgeneta"});
	...
	d1=d1.Define("goodmuonefc",efficiencies,{"goodgenpt","absgoodgeneta"});
	auto histo1 = d1.Histo2D({"histo1", "", neta, mineta, maxeta, npt, minpt, maxpt}, "absgoodgeneta", "goodgenpt", "goodmuonefc");
...
//	product->Multiply(Histo9);
	...
	c1->cd();
	c1->Draw();
	Histo1->SetTitle("P(passesglobal&&IDIP&&pfRelIso<0.15|gen)");
	...
	c3->cd();
	c3->Draw();
	product->SetTitle("P(pfRelIso<0.15|idip)*P(idip|passesglobal)*P(passesglobal|standalone)*P(standalone|tracker seeded)");

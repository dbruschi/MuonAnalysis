...
void isolationtnp(std::vector<std::string> names) {
	...
	TFile* histos=new TFile("zqthistos.root","RECREATE");
	histos->cd();
	...
	for (unsigned int i=1; i<=nqt; i++) {
		...
		std::string title("MCtruth Z q_{T}>");
		title+=std::to_string(binqtmin)+"&&Z q_{T}<"+std::to_string(binqtmax);
		std::string name2("TNP");
		name2+="_"+std::to_string(i);
		std::string title2("TNP Z q_{T}>");
		title2+=std::to_string(binqtmin)+"&&Z q_{T}<"+std::to_string(binqtmax);
		std::string name3("MCvsTNP");
		name3+="_"+std::to_string(i);
		std::string title3("MCvsTNP Z q_{T}>");
		title3+=std::to_string(binqtmin)+"&&Z q_{T}<"+std::to_string(binqtmax);

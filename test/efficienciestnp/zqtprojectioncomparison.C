...
void zqtprojectioncomparison(std::vector<std::string> names) {
	gStyle->SetOptStat(0);
	...
	Histo1->GetXaxis()->SetTitle("Z p_{T} probe projection");
	Histo1->Draw();
	TCanvas *c2=new TCanvas();
	c2->cd();
	c2->Draw();
	Histo2->SetTitle("TNP");
	Histo2->GetXaxis()->SetTitle("Z p_{T} probe projection");
	Histo2->Draw();
	TCanvas *c3=new TCanvas();
	c3->cd();
	c3->Draw();
	ratio->GetXaxis()->SetTitle("Z p_{T} probe projection");
	ratio->SetTitle("MC truth vs TNP");
	ratio->Draw();
	TFile* output=new TFile("zqtprojection.root","RECREATE");
	output->cd();

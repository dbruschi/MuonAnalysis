...
void triggermatchtnp(std::vector<std::string> names) {
	...
	d1=d1.Define("invariantmass",invariantmasses,{"triggerdenominatortag","triggerdenominatorprobe","Muon_pt","Muon_eta","Muon_phi","Muon_pt","Muon_eta","Muon_phi","Muon_mass"}).Define("passinginvariantmasses","invariantmass*(istrigger)").Define("failinginvariantmasses","invariantmass*(istrigger==false)");
	d1=d1.Define("absgoodgeneta","abs(goodgeneta)").Define("absgoodtracketa","abs(goodtracketa)").Define("absgoodstandeta","abs(goodstandeta)").Define("absgoodglobaleta","abs(goodglobaleta)").Define("absgoodidipeta","abs(goodidipeta)");
	...
	auto histo11 = d1.Histo3D({"passinginvariantmasses", "", neta, mineta, maxeta, npt, minpt, maxpt, nim, minim, maxim}, "absgoodidipeta","goodidippt","passinginvariantmasses");
	auto histo12 = d1.Histo3D({"failinginvariantmasses", "", neta, mineta, maxeta, npt, minpt, maxpt, nim, minim, maxim}, "absgoodidipeta","goodidippt","failinginvariantmasses");
	...
	TH3D* passinginvariantmasses=(TH3D*)histo11.GetPtr()->Clone(), *failinginvariantmasses=(TH3D*)histo12.GetPtr()->Clone();
	...
	c1->cd();
	c1->Draw();
	Histo1->SetTitle("P(passesglobal&&idip&&trigger match|gen)");
	...
	c3->cd();
	c3->Draw();
	product->SetTitle("P(trigger|idip)*P(idip|passesglobal)*P(passesglobal|standalone)*P(standalone|tracker seeded)");

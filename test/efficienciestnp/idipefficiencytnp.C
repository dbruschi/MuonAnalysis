...
void idipefficiencytnp(std::vector<std::string> names) {
	...
	d1=d1.Define("invariantmass",invariantmasses,{"idipdenominatortag","idipdenominatorprobe","Muon_pt","Muon_eta","Muon_phi","Muon_pt","Muon_eta","Muon_phi","Muon_mass"}).Define("passinginvariantmasses","invariantmass*(isidip)").Define("failinginvariantmasses","invariantmass*(isidip==false)");
	...
	c1->cd();
	c1->Draw();
	Histo1->SetTitle("P(passesglobal&&idip|gen)");
	...
	c3->cd();
	c3->Draw();
	product->SetTitle("P(idip|passesglobal)*P(passesglobal|standalone)*P(standalone|tracker seeded)");

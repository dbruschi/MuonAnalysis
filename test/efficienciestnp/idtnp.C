...
void idefficiencytnp(std::vector<std::string> names) {
	...
	d1=d1.Define("isid",isglobal,{"iddenominatorflag","six"});
	...
	d1=d1.Define("invariantmass",invariantmasses,{"iddenominatortag","iddenominatorprobe","Muon_pt","Muon_eta","Muon_phi","Muon_pt","Muon_eta","Muon_phi","Muon_mass"}).Define("passinginvariantmasses","invariantmass*(isid)").Define("failinginvariantmasses","invariantmass*(isid==false)");
	...
	auto histo7 = d1.Histo2D({"histo7", "", neta, mineta, maxeta, npt, minpt, maxpt}, "absgoodglobaleta", "goodglobalpt", "isid");
	...
	c1->cd();
	c1->Draw();
	Histo1->SetTitle("P(passesglobal&&id|gen)");
	...
	c3->cd();
	c3->Draw();
	product->SetTitle("P(id|passesglobal)*P(passesglobal|standalone)*P(standalone|tracker seeded)");

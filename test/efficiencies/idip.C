...
void idip(const char* name) {
	...
	c1->cd();
	c1->Draw();
	Histo1->SetTitle("P(global&&medium ID&&abs(d_{xy})<0.05|gen)");
	...
	c3->cd();
	c3->Draw();
	product->SetTitle("P(medium ID&&abs(d_{xy})<0.05|global)*P(global|standalone)*P(standalone|tracker seeded)");
	...
	Histo9->GetXaxis()->SetTitle("#eta");
	Histo9->GetYaxis()->SetTitle("p_{T} [GeV]");
	Histo9->Draw("colz");
	output->Write();
//	output->Close();
}

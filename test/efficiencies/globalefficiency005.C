...
auto goodgenvalue(RVec<float> &GenPart_pt, int &GenPart_postFSRLepIdx1, int &GenPart_postFSRLepIdx2, RVec<float> &GenPart_eta, RVec<float> &GenPart_phi, RVec<int> &GenPart_status) {
	...
	for (auto i=0U;i < GenPart_pt.size(); ++i) {
		if ((i==GenPart_postFSRLepIdx1)||(i==GenPart_postFSRLepIdx2)) {
			//if (abs(GenPart_eta[i])>2.4) continue;
			bool condition=true;
...
/*auto goodmuonbool(RVec<int> &Muon_genPartIdx, RVec<int> &vidx, RVec<bool> &Muon_isGlobal, RVec<float> &Muon_standpt, RVec<float> &Muon_pt) {
	RVec<float> v;
	for (auto i=0U;i < vidx.size(); ++i) {
		bool condition=false;
		for (auto j=0U; j < Muon_genPartIdx.size(); ++j) {
			if ((vidx[i]==Muon_genPartIdx[j])&&(Muon_isGlobal[j])) {
				condition=true;
			}
		}
...
auto goodmuonbool(RVec<float> &goodgeneta, RVec<float> &goodgenphi, RVec<float> &Muon_standeta, RVec<float> &Muon_standphi, RVec<bool> &Muon_isGlobal, RVec<float> &Muon_pt) {
	RVec<bool> v;
	for (auto i=0U; i<goodgeneta.size(); i++) {
		...
		for (auto j=0U; j<Muon_standeta.size(); ++j) {
			...
			if ((cand1.DeltaR(cand2)<0.05)) {condition=true;
				idx=j;
			}
		}
		if ((condition)&&(Muon_isGlobal[idx])) v.emplace_back(1);
		else v.emplace_back(0); 
	}
	return v;
}
...
auto isgoodtrack(RVec<float> &goodtracketa, RVec<float> &goodtrackphi, RVec<float> &Muon_eta, RVec<float> &Muon_phi, RVec<bool> &Muon_isStandalone, RVec<float> &Muon_standpt) {
	RVec<bool> v;
	for (auto i=0U;i < goodtracketa.size(); ++i) {
		...
		for (auto j=0U; j < Muon_eta.size(); ++j) {
			...
			if ((cand.DeltaR(cand2)<0.05)&&(Muon_isStandalone[j])) condition=true;
		}
...
auto goodmuon(RVec<float> &value, RVec<float> &goodgeneta, RVec<float> &goodgenphi, RVec<float> &Muon_standeta, RVec<float> &Muon_standphi, RVec<bool> &Muon_isStandalone, RVec<float> &Muon_standpt){
	RVec<float> v;
	for (auto j=0U; j < goodgeneta.size(); ++j) {
		bool condition=false;
		float Val;
		for (auto i=0U;i < Muon_standeta.size(); ++i) {
			...
			if ((Muon_isStandalone[i])&&(cand.DeltaR(cand2)<0.05)) {condition=true;
				Val=value[j];
			}
		}
		if (condition) v.emplace_back(Val);
...
auto goodmuonidx(RVec<float> &goodgeneta, RVec<float> &goodgenphi, RVec<float> &Muon_standeta, RVec<float> &Muon_standphi, RVec<bool> &Muon_isStandalone, RVec<float> &Muon_standpt){
	RVec<int> v;
	for (auto j=0U; j < goodgeneta.size(); ++j) {
		bool condition=false;
		float Val;
...
auto goodstand(RVec<int> &goodmuonidx, RVec<bool> &Muon_isGlobal, RVec<float> &Muon_pt) {
	RVec<bool> v;
...
void globalefficiency(const char* name) {
	...
	d1=d1.Define("goodtrack",isgoodtrack,{"goodtrackrealeta","goodtrackrealphi","Muon_standeta","Muon_standphi","Muon_isStandalone","Muon_standpt"});
	d1=d1.Define("goodstandpt",goodmuon,{"goodgenpt","goodgeneta","goodgenphi","Muon_standeta","Muon_standphi","Muon_isStandalone","Muon_standpt"}).Define("goodstandeta",goodmuon,{"goodgeneta","goodgeneta","goodgenphi","Muon_standeta","Muon_standphi","Muon_isStandalone","Muon_standpt"}).Define("goodstandphi",goodmuon,{"goodgenphi","goodgeneta","goodgenphi","Muon_standeta","Muon_standphi","Muon_isStandalone","Muon_standpt"}).Define("goodmuonidx",goodmuonidx,{"goodgeneta","goodgenphi","Muon_standeta","Muon_standphi","Muon_isStandalone","Muon_standpt"});
	d1=d1.Define("goodstand",goodstand,{"goodmuonidx","Muon_isGlobal","Muon_pt"});
